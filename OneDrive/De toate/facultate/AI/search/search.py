    ...
    while not pq.isEmpty():
        ...
        if current not in explored:
            ...
            for successor, action, cost in problem.getSuccessors(current):
                pq.push((successor, actions + [action]), problem.getCostOfActions(actions + [action]))
    return []
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic) -> List[Directions]:
    ...
    while not pq.isEmpty():
        ...
        if current not in explored:
            explored.add(current)
            for successor, action, cost in problem.getSuccessors(current):
                pq.push((successor, actions + [action]), problem.getCostOfActions(actions + [action]) + heuristic(successor, problem))
